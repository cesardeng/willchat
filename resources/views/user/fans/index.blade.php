        ...
        @if(count($fanList) > 0)
          @foreach($fanList as $key => $fan)
            <tr>
              ...
              <td>
                <a class="btn btn-primary btn-xs dialog-popup" href="{{ user_url('fans/editremark/'.$fan->id) }}"><i class="fa fa-edit"></i>修改备注</a>
                <a class="btn btn-primary btn-xs dialog-popup" href="{{ user_url('fans/moveto/'.$fan->id) }}"><i class="fa fa-hand-paper-o"></i>移动至分组</a>
              </td>
            </tr>
          @endforeach
        @else
